...
> **進行中（2026-07-28 起）。** 一次一顆，計畫本身不改；差異記在這裡，逐顆累加。
>
> **先修正兩處已被後續 ADR 作廢的計畫文字**（Phase 03 落地時反轉了前提）：
>
> - 標頭「關聯 ADR：沿用 **ADR 031**（框架）」——**框架 ADR 是 032**；031 是 ADAA
>   （Phase 06 先落地佔了號）。
> - 標頭 §1(b)、§2 前言、§3、§5 的「散射矩陣由 **R-Solver** 自產」「netlist 進
>   `tools/netlists/` 供 R-Solver 重產矩陣」——**整條流程不存在了**。ADR 032 之後
>   junction netlist 就是 Rust 資料（`Junction` + `JEl`），`S` 在 `calc_impedance`
>   由 MNA 數值構造。移植協定的 (b)+(c) 因此併成一步，而且驗證管道現成。
>
> **已交付**
>
> 1. **`ts-wdf`（§2.1）— PRD 026 / ADR 033，2026-07-28。** 落差三處：
>    - **op-amp 參數不沿用參考實作的 `(Ag=100, Ri=1e9, Ro=0.1)`**，改用 JRC4558
>      datasheet 的 `(3000, 5M, 75)`。理由可測量：`Ag=100` 同時壓掉 drive 掃程頂端
>      與 `C4` 的高頻衰減，而後者正是 §4.1 character pin 要求的「drive 轉大變暗」。
>      **政策化到後續每一顆**（ADR 033 §1）。
>    - **§2.7 的二極體選單帶 `(Is, n)` 而非只有 `Is`**，並修正 `1N34`：參考實作的
>      200 pA 配上矽的 `n` 會讓鍺削得比矽**還高**（與零件相反，且比流通的 1N34A
>      SPICE 模型小 1000 倍）。本專案用 `2.0e-7 / n=1.28`。另加第四檔 `LED`
>      （計畫只列三檔）——同樣是新 key，且是 TS 最常見的第二種改法。
>    - **Count 旋鈕預設 1.0 而非 1.906**：ideality 移到二極體表裡，count 只做
>      count。原廠聲音不變（`1N4148` 那格帶 `n=1.906`）。
>    - **§4.1 白箱判別改為兩側**：低頻由 `C3`、高頻由會移動的 `C4` 擋，中頻獨受
>      削平。單側可以用「曲線 + 濾波器」偽造，兩側加上會跟著旋鈕跑的轉角不行。
...
>
> 4. **`rat`（§2.5）— PRD 029，2026-07-28。** 沿用 `mxr-dist` 的 OUT 佈局。落差三處：
>    - **Filter 鈕做成反向**（往上轉 = 更暗），因為真實踏板就是那樣；家族其他 tone
>      控制一律相反，所以模組文件與面板名稱都寫明。
>    - **AC 分析必須把 `Ro` 留在算式裡**：這顆開到底的迴路增益 **< 1**（電阻要 1300×、
>      LM308 給 1000），家族其他顆「回授會把 `Ro` 除掉」的論證在這裡不成立。
>    - **oracle 測試的度量再修一次**：oracle *自身*的收斂要用「隱含電壓誤差」
>      `residual/(df/dv)` 表示；而可斷言的下限是 **f32**（`solve_newton` 回傳 f32，
>      半伏附近間距 6e-8 V）。
>    - **已知缺口**：不模擬 LM308 的轉速率限制，那是 RAT 音色的一部分且無法用調參補。
>
> 5. **`diode-clipper`（§2.6）— PRD 030，2026-07-28。** 平台件，四種接法
>    （Shunt / Series / Rectify / Feedback）× 三種器件 × 連續顆數。新增
>    `Ctl::Mode` + `Circuit::set_mode`（ADR 033 §4 已鋪好的路的第二個用例）。
>    兩件寫測試才發現的事：**輸出不在 root 節點上時要從 port 電流取樣**
>    （`i = (a−v)/R`），手推 `e − v` 在 series adaptor 上會**加**上壓降而非減去；
>    以及**測諧波要選整數週期的頻率**（187.5 Hz = 192 kHz/1024），否則基頻洩漏會蓋過
>    要量的效應。
>
> 6. **`king-of-tone`（§2.3）— PRD 031，2026-07-28。** 兩級兩 root。落差兩處：
>    - **不建前置線性增益級**（真實 Drive 電位器在那裡），改讓 `R10` 當 Drive 掃
>      22k–220k。設計選擇，非元件事實。
>    - **機制測試必須量在第一級**：第二級是硬並聯削波器，跑整顆會把一切壓到同一處、
>      正好蓋掉第一級存在的理由（第一版兩條測試因此只差 1–3 % 而失敗——量錯地方，
>      不是模型錯）。
>    - 同一個 junction，訊號改插在 **gain leg** 上就成了反相級——港口指派由 AC 分析
>      確認。
>
> **本 Phase 六顆全數落地。** 家族共用 junction 出現兩種 adapt 佈局、五顆共用，
> 沒有任何一份散射矩陣被寫下來過。
