...
> 4. **§2.1 的 makeup 校準點於 2026-07-29 重校（ADR 037）。** 原交付以 80 Hz–7.2 kHz
>    **頻帶平均**校 unity；平均跨過 7–9 dB 的 noon 凹陷，代數上把低頻 shelf 推到
>    unity 之上——五顆遷移 drive 的 40–160 Hz 絕對電平因此比遷移前高 +4～5 dB，
>    正是 §4.4 耳朵驗收不通過的主因（低頻溢出、錶頭爆、drive/level 救不回）。
>    改為「**noon 天花板 = unity**」：makeup ＝ noon 最大增益取負（bassman +1.63、
>    jcm800 +0.98、big-muff +4.05），noon 處處 ≤ 0 dB 如被動網路本身；重測後
>    低頻帶回到遷移前 0.7 dB 內，凹陷深度與旋鈕互動不變。
>
