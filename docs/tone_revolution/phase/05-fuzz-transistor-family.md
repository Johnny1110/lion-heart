...
> **落地修正框（2026-07-29）。** 本 Phase 已實作，實作時與本文有下列出入，以
> **ADR 035 + PRD 032/033/034** 為準：
>
> 1. **ADR 編號**：本文 §1、§5 寫「新開 ADR（暫定 032）」。032 早已被 Phase 03 的
>    WDF 可組合框架用掉，本 Phase 的 ADR 是 **035**。PRD 依主序列取號 **032–034**。
> 2. **新模組不叫 `blocks::wdf` 的一部分**：本文 §5 寫「`blocks::wdf`（或
>    `blocks::transistor`）」。實作選了後者，理由寫在 ADR 035 §1——這一族一個波都沒有，
>    塞進 WDF 模組會讓那個詞失去意義。
> 3. **Rangemaster 不是「標量」Newton**：本文 §1(b)/§2.3 說是標量迭代。雙極電晶體是
>    **雙端口**非線性，最少要兩個未知數；實作解**三節點**（基極/射極/集極）。範圍仍然
>    小、成本仍然有界，但不是標量。見 ADR 035 §1、PRD 033 §2.1。
> 4. **參考實作有兩個數值問題，落地時才量得出來**（ADR 035 §3）：Big Muff 的回授電流
>    注入在**直流**戴維南電阻（100 kΩ）而非交流值（9.09 kΩ），每級差 6.5 倍增益；
>    Rangemaster 的 `*1e16 - 5e5 - 1.0` 輸出縮放是**繪圖程式的殘留**，而且它對 PNP
>    餵 NPN 方程。兩者都不能當參考實作，本專案自行推導。
> 5. **鍺參數不照抄**（本文 §2.3 已預留「實機參數再校準」）：`Is = 1e-7 A`（不是矽的
>    10 fA）、`βF = 100`、`βR = 2`。工作點由手算釘住，見 PRD 033 §3.1。
> 6. **Fuzz Face 的「選配精修」有做**：Germanium / Silicon 型號選擇（PRD 034）。
>    索引 0 逐字等於既有常數，所以四條既有 character 測試一條都沒重新 pin。
> 7. **成本高於預期**：本文 §4.2 預期「與 `sd1` 同量級」。`big-muff` 2.38× screamer、
>    `rangemaster` **4.55×**（家族最貴，~12 % of deadline）。數字與已做的最佳化見
>    PRD 033 §3.3 與 `docs/benchmarks.md`。
