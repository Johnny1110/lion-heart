...
ADR 009 (mod family), ADR 018 (global tempo: the BPM source that feeds this),
white paper §7 (M6 "on stage"), §4.3 (normalized params behind a `Range`)

> **Merge reconciliation (2026-07-21).** This ADR and ADR 018 were authored in
> parallel on two machines for the same feature area, and reconciled at merge
> time into one combined design. This ADR owns the **sync target**: the stepped
> `sync` note-division selector (`lh_core::tempo::SYNC_DIVISIONS`), the engine's
> `ChainHandle::apply_tempo_sync`, and locking a delay's `time` / a tremolo's
> `rate`. ADR 018 owns the **BPM source** — one session-owned tempo fed by tap,
> MIDI clock, and (plugin) host transport — which drives `apply_tempo_sync`.
> The two together supersede the pieces each described in isolation: the source
> is no longer "tap + typed only" (this ADR's original scope) and the sync is no
> longer a boolean `subdivision` follow (ADR 018's original scope). The plugin
> is now host-transport-driven for the delay `time`, not inert.
